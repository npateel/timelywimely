...
if __name__ == "__main__":
    ...
    for year in years:
        ...
        json_path = os.path.join(json_path, "nq_open_comparison.json")
        with open(json_path) as jf:
            dicts.append(json.load(jf))

    df = dfOverTime(dicts, years)
    df.to_csv("../haystack/OpenOvertime.csv")

    df = dfOverTime(dicts, years, withGT = True)
    df.to_csv("../haystack/OpenBoth.csv")

    df = dfGTcompare(dicts, years)
    df.to_csv("../haystack/OpenGT.csv")
